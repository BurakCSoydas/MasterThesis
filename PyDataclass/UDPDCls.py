...
class DMDataclass:
    """DMData groups data the dm is making use of and
    makes it easier to work with. Using @dataclass decorator
    for optimization."""

    timestamp: float = field(default_factory=float)
    egoLane: int = field(default_factory=int)
    egoPosX: float = field(default_factory=float)
    egoSpeed: float = field(default_factory=float)
    egoAcc: float = field(default_factory=float)
    egoLength: float = field(default_factory=float)
    targetLane: int = field(default_factory=int)
    targetPosX: float = field(default_factory=float)
    targetSpeed: float = field(default_factory=float)
    targetAcc: float = field(default_factory=float)
    targetLength: float = field(default_factory=float)
    targetWidth: float = field(default_factory=float)
...
class UDP:
    ...
    def respondToESMINI(self, id=0, brake=0.0, throttle=1.0, steeringAngle=0.0):
        ...
        if responseMSG is not None:
            ...
            print("responseToESMINI is None")
