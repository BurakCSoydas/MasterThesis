...
class DM(UDP.UDP):
    def __init__(
        ...
        if self.log:
            ...
            self.filePath: str = (
                self.outputPath + "Log-PyList" + mode + timestamp + ".csv"
            )
    ...
    def runDM(self):
        ...
        if self.log:
            logString = str(self.UDP.DMData[0]) + ","
            logString += "{:.6g}".format(self.prevDistEgoToTarget) + ","
            logString += "{:.6g}".format(self.UDP.DMData[3]) + ","
            ...
            logString += "{:.4g}".format(self.UDP.DMData[6]) + ","
            logString += "\n"
...
def main():
    ...
    for _ in range(args.numberIterations):
        ...
        if (dm.response > 0) and (dm.UDP.DMData[1] == dm.UDP.DMData[6]):
    ...
    print(f"ENDPOSITION: {dm.UDP.DMData[2]}")
    if args.log:
        dm.logFile.close()
    dm.close
